...
// Get Veeam Backup Infrastructure States
$app->get('/plugin/VeeamPlugin/backupinfrastructurestates', function ($request, $response, $args) {
    $VeeamPlugin = new VeeamPlugin();
    if ($VeeamPlugin->auth->checkAccess($VeeamPlugin->config->get("Plugins", "VeeamPlugin")['ACL-READ'] ?? "ACL-READ")) {
        $VeeamPlugin->GetBackupInfrastructureStates();
    }
    $response->getBody()->write(jsonE($GLOBALS['api']));
    return $response
        ->withHeader('Content-Type', 'application/json;charset=UTF-8')
